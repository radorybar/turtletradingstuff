...
//Do not forget to setup variables evaluated here as a static, to remember last state, if not counted twice for a tick
   if(TickCounted != Tick)
   {
      ...
      MA1 = iMA(_SYMBOL, getHigherTimeframe(_TIMEFRAME), 10, 0, MODE_SMA, PRICE_CLOSE, 1);
      MA2 = iMA(_SYMBOL, getHigherTimeframe(_TIMEFRAME), 10, 0, MODE_SMA, PRICE_CLOSE, 2);
      MACDDiff = iCustom(_SYMBOL, _TIMEFRAME, "MACD_RR2", 2, 1);
      MACDOsc = iCustom(_SYMBOL, _TIMEFRAME, "MACD_RR2", 12,26,9,_OSCMULTIPL, _OSCPERIOD, 3, 1);
      MACDSignal = iCustom(_SYMBOL, _TIMEFRAME, "MACD_RR2",1, 1);
/*
