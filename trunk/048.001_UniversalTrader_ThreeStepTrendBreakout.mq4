...
//   int      _TRADING_HOURS[]  = {3,7,};
   ...
   static datetime ZIGZAGTraded = 0;
...
//Do not forget to setup variables evaluated here as a static, to remember last state, if not counted twice for a tick
   if(TickCounted != Tick)
   {
      ...
      LowerZIGZAGTime3 = getNthZIGZAGTime(_SYMBOL, _TIMEFRAME, false, 3);
      ...
      UpperZIGZAGShift3 = iBarShift(_SYMBOL, _TIMEFRAME, UpperZIGZAGTime3);
      ...
      LowerZIGZAGShift3 = iBarShift(_SYMBOL, _TIMEFRAME, LowerZIGZAGTime3);

      MACDDiff = iCustom(_SYMBOL, _TIMEFRAME, "MACD+HistogramDiff+SignalDiff", 2, 1);
      MACDOsc = iCustom(_SYMBOL, _TIMEFRAME, "MACD+HistogramDiff+SignalDiff", 12,26,9,_OSCMULTIPL, 3, 1);
      MACDHistogram = iMACD(_SYMBOL, _TIMEFRAME, 12, 26, 9, PRICE_CLOSE, MODE_MAIN, 1);
      MACDSignal = iMACD(_SYMBOL, _TIMEFRAME, 12, 26, 9, PRICE_CLOSE, MODE_SIGNAL, 1);
...
//downtrend
//1.zigzag nizsie ako spojnica 3. a 2. zigzagu
//prerazenie zigzag pojnice vrcholov na nizsom timeframe
//macd ako potvrdenie
//         if(ZIGZAGTraded < UpperZIGZAGTime1)
         if(UpperZIGZAG3 > UpperZIGZAG2)
         if((UpperZIGZAG3 - UpperZIGZAG2)/(UpperZIGZAGShift3 - UpperZIGZAGShift2) < (UpperZIGZAG2 - UpperZIGZAG1)/(UpperZIGZAGShift2 - UpperZIGZAGShift1))
         if(MACDHistogram > MACDSignal)
            result = 1;

//         if(result = 1)
//            ZIGZAGTraded = UpperZIGZAGTime1;
