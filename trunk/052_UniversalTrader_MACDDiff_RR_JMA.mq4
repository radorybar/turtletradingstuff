...
//Do not forget to setup variables evaluated here as a static, to remember last state, if not counted twice for a tick
   if(TickCounted != Tick)
   {
      ...
      MACDDiff = iCustom(_SYMBOL, _TIMEFRAME, "MACD_RR_JMA", 2, 1);
      MACDOsc = iCustom(_SYMBOL, _TIMEFRAME, "MACD_RR_JMA", 3, 1);
//      MACDSignalDiff = iCustom(_SYMBOL, _TIMEFRAME, "MACD_RR_JMA", 5, 1);
...
//         if(MACDOsc > 0)
         if(MACDDiff > 0)
//         if(iLow(_SYMBOL, getHigherTimeframe(_TIMEFRAME), 1) > iLow(_SYMBOL, getHigherTimeframe(_TIMEFRAME), 3))
//         if(MACDSignalDiff > 0)
            result = 1;
...
//         if(MACDOsc < 0)
         if(MACDDiff < 0)
//         if(iHigh(_SYMBOL, getHigherTimeframe(_TIMEFRAME), 1) < iHigh(_SYMBOL, getHigherTimeframe(_TIMEFRAME), 3))
//         if(MACDSignalDiff < 0)
            result = 1;
