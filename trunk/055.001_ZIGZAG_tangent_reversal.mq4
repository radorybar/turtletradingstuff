...
//if order for this strategy already exists - do not check this strategy for open         
/*
...
//   int      _TRADING_HOURS[]  = {3,7,};
   ...
   static double  ZIGZAGTangentUpper;
   static double  ZIGZAGTangentLower;
   ...
   static int UpperZIGZAG1Bar;
   static int LowerZIGZAG1Bar;
...
//Do not forget to setup variables evaluated here as a static, to remember last state, if not counted twice for a tick
   if(TickCounted != Tick)
   {
      ...
      UpperZIGZAG1Bar = iBarShift(_SYMBOL, _TIMEFRAME, UpperZIGZAG1Time);
      LowerZIGZAG1Bar = iBarShift(_SYMBOL, _TIMEFRAME, LowerZIGZAG1Time);
/*      
...
*/
/*
      UpperZIGZAG1 = getLastFractalValue(_SYMBOL, _TIMEFRAME, true);
      LowerZIGZAG1 = getLastFractalValue(_SYMBOL, _TIMEFRAME, false);
      UpperZIGZAG1Time = getLastFractalTime(_SYMBOL, _TIMEFRAME, true);
      LowerZIGZAG1Time = getLastFractalTime(_SYMBOL, _TIMEFRAME, false);
      UpperZIGZAG1Bar = iBarShift(_SYMBOL, _TIMEFRAME, UpperZIGZAG1Time);
      LowerZIGZAG1Bar = iBarShift(_SYMBOL, _TIMEFRAME, LowerZIGZAG1Time);
*/
      if(OpenNewBar(_TIMEFRAME))
      {
//         if(getLastFractalTime(_SYMBOL, _TIMEFRAME, false) > UpperZIGZAG1Time)
         {
            ZIGZAGTangentUpper = 1000;
            for(i = UpperZIGZAG1Bar - 1; i > 0; i--)
            {
      //         if(iFractals(_SYMBOL, _TIMEFRAME, MODE_UPPER, i) > 0)
               if((UpperZIGZAG1 - High[i])/(UpperZIGZAG1Bar - i) < ZIGZAGTangentUpper)
                  ZIGZAGTangentUpper = (UpperZIGZAG1 - High[i])/(UpperZIGZAG1Bar - i);
            }
         }
//         if(getLastFractalTime(_SYMBOL, _TIMEFRAME, true) > LowerZIGZAG1Time)
         {
            ZIGZAGTangentLower = -1000;
            for(i = LowerZIGZAG1Bar - 1; i > 0; i--)
            {
      //         if(iFractals(_SYMBOL, _TIMEFRAME, MODE_LOWER, i) > 0)
               if((LowerZIGZAG1 - Low[i])/(LowerZIGZAG1Bar - i) > ZIGZAGTangentLower)
                  ZIGZAGTangentLower = (LowerZIGZAG1 - Low[i])/(LowerZIGZAG1Bar - i);
            }
         }
         ObjectCreate(StringConcatenate(TimeCurrent(), ""), OBJ_ARROW, 0, TimeCurrent(), UpperZIGZAG1 - ZIGZAGTangentUpper*UpperZIGZAG1Bar);
         ObjectSet(StringConcatenate(TimeCurrent(), ""), OBJPROP_ARROWCODE, 159);
         ObjectSet(StringConcatenate(TimeCurrent(), ""), OBJPROP_COLOR, Blue);
         ObjectCreate(StringConcatenate(TimeCurrent(), "A"), OBJ_ARROW, 0, TimeCurrent(), LowerZIGZAG1 - ZIGZAGTangentLower*LowerZIGZAG1Bar);
         ObjectSet(StringConcatenate(TimeCurrent(), "A"), OBJPROP_ARROWCODE, 159);
         ObjectSet(StringConcatenate(TimeCurrent(), "A"), OBJPROP_COLOR, Red);
      }
/*
...
/*
         if(Ask <= LowerZIGZAG1 + ZIGZAGTangentLower*LowerZIGZAG1Bar)
         if(LastAsk > LowerZIGZAG1 + ZIGZAGTangentLower*LowerZIGZAG1Bar)
            result = 1;
*/

         if(LastZIGZAGTradedTime <  LowerZIGZAG1Time)
         if(Ask >= UpperZIGZAG1 - ZIGZAGTangentUpper*UpperZIGZAG1Bar)
         if(LastAsk < UpperZIGZAG1 - ZIGZAGTangentUpper*UpperZIGZAG1Bar)
            result = 1;
            
//       double a = UpperZIGZAG1 + ZIGZAGTangentUpper*UpperZIGZAG1Bar;         
//       Comment(UpperZIGZAG1 + "\n" +  ZIGZAGTangentUpper  + "\n" + UpperZIGZAG1Bar + "\n" + a);

         if(result == 1)
         {
//            ObjectCreate(StringConcatenate(TimeCurrent(), ""), OBJ_ARROW, 0, TimeCurrent(), Ask);
...
/*
         if(Bid >= ZIGZAGTangentUpper)
         if(LastBid < ZIGZAGTangentUpper)
            result = 1;
*/

         if(LastZIGZAGTradedTime <  UpperZIGZAG1Time)
         if(Bid <= LowerZIGZAG1 - ZIGZAGTangentLower*LowerZIGZAG1Bar)
         if(LastBid > LowerZIGZAG1 - ZIGZAGTangentLower*LowerZIGZAG1Bar)
            result = 1;

//       double a = LowerZIGZAG1 + ZIGZAGTangentLower*LowerZIGZAG1Bar;         
//       Comment(LowerZIGZAG1 + "\n" +  ZIGZAGTangentLower  + "\n" + LowerZIGZAG1Bar + "\n" + a);
...
/*
         if(getOrderProfitByMAGICNUMBER(_MAGICNUMBER) > 0)
         if(Bid > ZCBUpper)
         if(LastBid < ZCBUpper)
         if(ZCBUpper > 0.0)
            result = 1;
...
//         if(getOrderProfitByMAGICNUMBER(_MAGICNUMBER) > 20)
//         if(LastZIGZAGTradedTime < UpperZIGZAG1Time)
//            result = 1;
