...
//   if(OpenNewBar(_TIMEFRAME))
   {
      ...
      ZCBUpper = iCustom(_SYMBOL, _TIMEFRAME, "051.001_ZIGZAG_Channel_Breakout", 0, 1);
      ZCBLower = iCustom(_SYMBOL, _TIMEFRAME, "051.001_ZIGZAG_Channel_Breakout", 1, 1);
      ZIGZAGChannelUp = iCustom(_SYMBOL, _TIMEFRAME, "051.006_ZIGZAG_Channel_Breakout", 3, 0);
      ZIGZAGChannelLow = iCustom(_SYMBOL, _TIMEFRAME, "051.006_ZIGZAG_Channel_Breakout", 4, 0);
...
//      Comment(LongStop + "\n" + ShortStop);
//       Comment(ZCBUpper + "\n" + ZCBLower + "\n" + ZIGZAG);
/*get last zigzag value
      for(i = 1; i < Bars; i++)
      {
         ZIGZAG = iCustom(_SYMBOL, _TIMEFRAME, "ZIGZAG_Channel_Breakout", 2, i);
         if(ZIGZAG > 0)
            break;
      }
*/      
