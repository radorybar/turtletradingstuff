...
//+------------------------------------------------------------------+
int SummaryReportInPoints()
  {
//----
   handle=FileOpen("_AccountHistory.txt",FILE_CSV|FILE_WRITE,'\t');
   handle_current=FileOpen("_AccountOpen.txt",FILE_CSV|FILE_WRITE,'\t');
   double initial_deposit=CalculateInitialDepositInPoints();
   CalculateSummaryInPoints(initial_deposit);
   WriteReportInPoints("_Summary.txt");
//----
   if(handle>0) FileClose(handle);
   
   if(handle_current>0) FileClose(handle_current);
...
//|                                                                  |
//+------------------------------------------------------------------+
void CalculateSummaryInPoints(double initial_deposit)
  {
...
//----
   ...
   FileWrite(handle_current,"Order","Open time","Type","Lots","Symbol","Open price","StopLoss","TakeProfit","Close time","Close price","Profit in points","Balance in points","Comment","Magic");
   ...
   for(i=0; i<OrdersTotal(); i++)
     {
      ...
      profit=(close_price-open_price)/point_value;
      if(type==OP_SELL) profit=-profit;
      balance+=profit;
      //---- output trade line
      command="sell";
      if(type==OP_BUY) command="buy";
      if(handle>0) FileWrite(handle_current,OrderTicket(),TimeToStr(OrderOpenTime()),command,OrderLots(),order_symbol,open_price,OrderStopLoss(),OrderTakeProfit(),TimeToStr(OrderCloseTime()),close_price,profit,balance,OrderComment(),OrderMagicNumber());
      //---- drawdown check
      if(maxpeak<balance)
        {
         drawdown=maxpeak-minpeak;
