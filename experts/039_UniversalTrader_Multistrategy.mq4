...
//------------------------------------------------------------------
//------------------------------------------------------------------
int   _ALL_STRATEGIES                  = 27;
int   _ACTIVE_STRATEGIES[]             = {26,27};
...
// MACD inverse signal
   if(_STRATEGY == Strategy_025(_GET_STRATEGY_NUMBER))
      return(Strategy_025(_COMMAND));
// MACD zero level cross
   if(_STRATEGY == Strategy_026(_GET_STRATEGY_NUMBER))
      return(Strategy_026(_COMMAND));
// MACD signal cross
   if(_STRATEGY == Strategy_027(_GET_STRATEGY_NUMBER))
      return(Strategy_027(_COMMAND));
...
//------------------------------------------------------------------//------------------------------------------------------------------
double Strategy_025(int _COMMAND)
{
...
//         break;
         ...
         MACDHistogram1 = iMACD(_SYMBOL, _TIMEFRAME, 26, 13, 9, PRICE_CLOSE, MODE_MAIN, 1);
         MACDSignal1 = iMACD(_SYMBOL, _TIMEFRAME, 26, 13, 9, PRICE_CLOSE, MODE_SIGNAL, 1);
         MACDHistogram2 = iMACD(_SYMBOL, _TIMEFRAME, 26, 13, 9, PRICE_CLOSE, MODE_MAIN, 2);
         MACDSignal2 = iMACD(_SYMBOL, _TIMEFRAME, 26, 13, 9, PRICE_CLOSE, MODE_SIGNAL, 2);
         
         if(MACDSignal2 < MACDHistogram2)
         if(MACDSignal1 > MACDHistogram1)
            result = 1;
...
//------------------------------------------------------------------//------------------------------------------------------------------
double Strategy_026(int _COMMAND)
{
   int      _STRATEGY_NUMBER  = 26;
   int      _MAGICNUMBER      = _STRATEGY_NUMBER;
...
//------------------------------------------------------------------//------------------------------------------------------------------
double Strategy_027(int _COMMAND)
{
   int      _STRATEGY_NUMBER  = 27;
   int      _MAGICNUMBER      = _STRATEGY_NUMBER;
