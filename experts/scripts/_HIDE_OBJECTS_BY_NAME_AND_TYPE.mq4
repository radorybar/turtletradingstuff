...
For example:
*/
extern string  hide_on_Partial_name  = "";
...
//----
	...
	for(j = 0; j < 24; j++)
		if(u[j])
			w1 = true;
	if(!w1&&StringLen(hide_on_Partial_name) == 0)
		for(int i = k-1; i >= 0; i--) //удаляем все//delte ALL
		{
			s = ObjectName(i);
//			ObjectDelete(s);
         ObjectSet(s, OBJPROP_TIMEFRAMES, EMPTY);
		}
		else
		{
			if(StringLen(hide_on_Partial_name) != 0) 
				for(i = k-1;i >= 0; i--)//удаляем партию//delete partial 
				{
					s = ObjectName(i);
					if(StringFind(s,hide_on_Partial_name,0) >= 0)
//						ObjectDelete(s);
                  ObjectSet(s, OBJPROP_TIMEFRAMES, EMPTY);
				}
