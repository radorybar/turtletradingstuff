...
{
// Intraday-MikrotrendsBreakout+Indicators+RiskManagement
...
//Do not forget to setup variables evaluated here as a static, to remember last state, if not counted twice for a tick
   ...
   switch(_COMMAND)
   {
      case _OPEN_LONG:
      {
         static bool buy_breakout = false;
         static datetime buy_breakout_time = 0;
...
//            break;
         ...
         //Su uz dva ZIGZAG vrcholy v ramci dna?
         //Ich spojnica je klesajuca?
         //Doslo k prelomeniu ich spojnice?
         //sme blizsie k poslednemu dnu ako max risk?
         //event. MACD konvergencia? MACD indikator? CCI konvergencia? CCI indikator?

         //pockat na nasledujuci spodny fraktal - fraktal po vyskyte breakout
         //cena uz samozrejme nemusi byt nad/pod breakout
         //ukoncit pri opacnom fraktale

//breakout conditions
         if(!buy_breakout)
         {
            EdgePrice = UpperZIGZAG1 - UpperZIGZAGShift1 * (UpperZIGZAG2 - UpperZIGZAG1) / (UpperZIGZAGShift2 - UpperZIGZAGShift1);

//Last ZIGZAG top is older than last ZIGZAG bottom - last ZIGZAG is bottom and price should revers UP
//            if(LowerZIGZAGTime1 > UpperZIGZAGTime1)
//We have already two ZIGZAG tops today
//            if(UpperZIGZAGTime2 > TodayStart)
//Previous ZIGZAG top is higher than last ZIGZAG top - trend is DOWN
//            if(UpperZIGZAG2 > UpperZIGZAG1)
//If price breaks last two ZIGZAG tops trendline by _BREAKOUTTHRESSHOLD points
            if(Ask > EdgePrice + _BREAKOUTTHRESSHOLD*Point)
//if last ZIGZAG low is not too far - SL not higher than MAX risk
//            if(LowerZIGZAG1 >= Bid - _MAXRISK*Point)
//MACD Divergence on ZIGZAG tops - ZIGZAGTOP2 > ZIGZAGTOP1 && MACDHISTOGRAM2 < MACDHISTOGRAM1
 //           if(UpperMACDHistogram2 < UpperMACDHistogram1)
//MACD Divergence on ZIGZAG bottoms - ZIGZAGBOTTOM2 > ZIGZAGBOTTOM1 && MACDHISTOGRAM2 < MACDHISTOGRAM1
//Previous ZIGZAG bottom is higher than last ZIGZAG
            if(LowerMACDHistogram2 < LowerMACDHistogram1)
            if(LowerMACDHistogram2 < 0)
            if(LowerZIGZAG2 > LowerZIGZAG1)   
//If there was no order today
//            if(OrderSelect(1, SELECT_BY_POS, MODE_HISTORY) == false || (OrderSelect(1, SELECT_BY_POS, MODE_HISTORY) == true && OrderOpenTime() < TodayStart))
            {
               buy_breakout = true;
               buy_breakout_time = TimeCurrent();
            }
         }
//wait on order open conditions after breakout
         else
         {
//Last bottom Fractal was created after breakout - pullback swing
//            if(LowerFractalTime1 > buy_breakout_time)
//Last upper Fractal was created after breakout - pullback swing, but considering, when top fractal is recognized, it is already bottom
//            if(UpperFractalTime1 > buy_breakout_time)
//If breakout happened after last zigzag
//            if(buy_breakout_time > LowerZIGZAGTime1)
//If upper zigzag appears, probably there is a bottom now
//            if(buy_breakout_time < UpperZIGZAGTime1)
//If lower zigzag appears after breakout, probably there was a bottom now
            if(buy_breakout_time < LowerZIGZAGTime1)
            {
               result = 1;
               buy_breakout = false;
      ...
      case _OPEN_SHORT:
      {
         static bool sell_breakout = false;
         static datetime sell_breakout_time = 0;
...
//Last ZIGZAG bottom is older than last ZIGZAG top - last ZIGZAG is top and price should revers DOWN
//         if(LowerZIGZAGTime1 < UpperZIGZAGTime1)
//We have already two ZIGZAG bottoms today
//         if(LowerZIGZAGTime2 > iTime(_SYMBOL, PERIOD_D1, 0))
//Previous ZIGZAG bottom is lower than last ZIGZAG bottom - trend is UP
//         if(LowerZIGZAG2 < LowerZIGZAG1)
//If price breaks last two ZIGZAG bottoms trendline by _BREAKOUTTHRESSHOLD points
         if(Bid < EdgePrice - _BREAKOUTTHRESSHOLD*Point)
//if last ZIGZAG top is not too far - SL not higher than MAX risk
//         if(UpperZIGZAG1 <= Ask + _MAXRISK*Point)
//MACD Divergence on ZIGZAG bottoms - ZIGZAGBOTTOM2 < ZIGZAGBOTTOM1 && MACDHISTOGRAM2 > MACDHISTOGRAM1
//         if(LowerMACDHistogram2 > LowerMACDHistogram1)
//MACD Divergence on ZIGZAG tops - ZIGZAGTOP2 < ZIGZAGTOP1 && MACDHISTOGRAM2 > MACDHISTOGRAM1
//Previous ZIGZAG top is lower than last ZIGZAG
//         if(UpperMACDHistogram2 > UpperMACDHistogram1)
//         if(UpperZIGZAG2 < UpperZIGZAG1)
//Last top Fractal was created after ZIGZAG high - pullback swing
//         if(UpperFractalTime1 > UpperZIGZAGTime1)
//MACD Divergence on ZIGZAG bottoms - ZIGZAGBOTTOM2 > ZIGZAGBOTTOM1 && MACDHISTOGRAM2 < MACDHISTOGRAM1
//Previous ZIGZAG bottom is higher than last ZIGZAG
//            if(UpperMACDHistogram2 > UpperMACDHistogram1)
//            if(UpperMACDHistogram2 > 0)
//            if(UpperZIGZAG2 < UpperZIGZAG1)   
            {
               sell_breakout = true;
               sell_breakout_time = TimeCurrent();
            }
         }
//wait on order open conditions after breakout
         else
         {
//If lower zigzag appears after breakout, probably there was a bottom now
            if(sell_breakout_time < UpperZIGZAGTime1)
            {
               result = 1;
               sell_breakout = false;
            }
